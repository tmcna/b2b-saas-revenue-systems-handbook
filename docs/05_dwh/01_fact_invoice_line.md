...
### なぜ「請求書単位」ではなく「明細行単位」か

請求書を1レコードにまとめると「この月の請求合計はいくらか」には答えられますが、「プランAと追加オプションの売上比率は？」には答えられません。明細行を粒度にすることで、`product_id` をキーにした商品別・プラン別の分析が可能になります。

商品ディメンション（[dim_product](./05_dim_product.md)）との結合も、明細行単位であって初めて意味を持ちます。

### service_start_date / service_end_date を持つ理由

年間前払い契約では、`invoice_date`（1月1日に請求書を発行）と`service_end_date`（12月31日までのサービス対価）が大きく離れます。この2つの日付を別カラムとして持つことで、「いつ請求したか」と「いつまでのサービス対価か」の両方で集計できます。

収益認識は請求日ではなくサービス提供期間に基づきます。前払い年間契約の Revenue を月次で按分する場合、このカラムがなければ計算できません。

### `is_credit_note` をフィルタせずフラグで持つ理由

クレジットノート（返金・値引き）を削除してしまうと、「返金が多い商品はどれか」「値引き交渉が発生した案件の傾向」を分析できなくなります。フラグとして保持することで、「正味の売上（クレジットノートをネットアウトした額）」と「発行した請求書の総額（クレジットノートを含まない）」の両方を目的に応じて集計できます。
