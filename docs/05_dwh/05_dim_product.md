...
商品の価格は変わります。値上げ・キャンペーン割引・顧客ごとの個別料金——これらはすべて「いくらで請求したか」というファクトです。ディメンションに `unit_price` を持つと、値上げ後に `dim_product` を更新した瞬間に、値上げ前のすべての `fact_invoice_line` が値上げ後の価格で参照されてしまいます。

「この商品の単価はいくらか」ではなく「この明細行ではいくらで請求したか」が正しい問いです。価格は `fact_invoice_line.unit_price` に属します。

### `billing_model` を持つ理由

固定料金・シートライセンス・従量課金では、収益の伸び方の構造が根本的に異なります。従量課金は使用量に応じてMRRが変動し、予測が難しくなります。`billing_model` を分析軸として持つことで、「課金モデル別のNRR比較」や「従量課金顧客の売上予測精度」を分析できます。

### `is_addon` を持つ理由

アドオン（追加オプション）はベースプランとは独立して解約・変更されます。アドオンを含めた売上と、ベースプランのみの売上を分けて集計することで、「アドオン導入率と解約率の相関」や「アドオンなしで契約した顧客のLTV」を分析できます。
