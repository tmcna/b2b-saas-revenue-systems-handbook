...
受注した見積だけを記録すると、「受注率の分析」ができません。受注率は「提案した件数に対して受注した件数の割合」であり、失注レコードが存在しないと分母を計算できません。

`quote_status`（won / lost / draft / expired）を持ち、すべての見積を取り込むことで、「プランA を提案した案件の受注率は？」「割引率 20% 超の案件はそうでない案件より受注しやすいか？」といった分析が可能になります。

### 商談（opportunity）と見積を両方持つ理由

1つの商談に複数の見積が作成されることがあります（提案のバリエーション、再提案など）。`opportunity_id` を持つことで「商談単位の最終受注額」を集計でき、`quote_id` を持つことで「どのバージョンの見積が最終的に受注されたか」を追跡できます。

### PLG では使用しないことの明示

PLG（セルフサービス）では見積プロセスが存在しないため、このテーブルはデータが入りません。SLGとPLGの混在モデルでは、`fact_quote_line` は SLG 顧客のみを対象とした分析ツールとして位置づけます。
