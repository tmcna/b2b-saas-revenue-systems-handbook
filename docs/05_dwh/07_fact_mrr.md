...
### なぜ fact_invoice_line から MRR を計算しないか

`fact_invoice_line` は「請求書を発行した事実」の記録です。年間前払い契約では、1月1日に12ヶ月分の請求書が発行されます。この `invoice_date` で月次集計すると、1月に売上が全額計上され、2〜12月は0になります。

MRR は「今継続している収益の水準」であり、請求の発生タイミングには依存しません。MRR の計算元はサブスクリプションの状態（どの顧客が、どのプランで、いつまで契約しているか）です。

```
年間前払い 120万円（1月請求）の場合：

  fact_invoice_line で集計  → 1月: 120万、2月: 0、3月: 0 ...（請求ベース）
  fact_mrr で集計           → 1月: 10万、2月: 10万、3月: 10万 ...（契約ベース）
```

### なぜ月次か（日次でないか）

SaaS 企業の経営判断の時間軸は月次です。日次スナップショットにするとデータ量が30倍になり、計算コストとストレージコストが増大します。日中のサブスクリプション変動は意思決定に影響しないため、月次で十分な粒度です。

### `mrr_prior_month` をファクトテーブルに持つ理由

MRR の変動カテゴリ（New/Expansion/Churn など）の計算には前月の MRR が必要です。これを毎回 self-join で取得することは可能ですが、アドホック分析のたびに JOIN するとクエリが複雑になります。`mrr_prior_month` を事前に計算して持つことで、`mrr - mrr_prior_month` の単純な差分計算で変動額が求まります。
...
方法Aがサブスクリプションビジネスの実態をより正確に反映します。
