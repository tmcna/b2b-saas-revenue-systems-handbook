...
### なぜ取引ファクトでなくスナップショットファクトか

「先月末時点で30日超の未収金はいくらか」という問いへの答えは、個々の入金イベントを積み上げても計算できません。請求日・入金日・部分入金・クレジットノート——これらを毎回集計して「その時点での残高」を求めるクエリは複雑になり、結果も遅くなります。

月末時点の状態をスナップショットとして保存しておけば、`WHERE snapshot_date = '2024-01-31'` の単純なフィルタで取り出せます。「3ヶ月前と比べて未収金が増えているか」も2つのスナップショットを比較するだけです。

### 未収金をバケットで持つ理由

貸倒引当金の計算は、経過日数に応じて引当率が変わります（例：30日以内は0%、31〜60日は10%、61〜90日は30%、91日超は100%）。この計算のために、個々の請求書ごとの経過日数を保持するより、財務部門が使う分類（バケット）に合わせた形で集計して持つ方が実用的です。個別の請求書レベルの詳細が必要な場合は、`fact_invoice_line` と `fact_payment` を結合して計算します。

### 月次スナップショットの理由

日次スナップショットにするとデータ量が30倍になります。売掛金管理の実務的な時間軸は月次（月末締め、翌月回収目標）であり、日次の変動を追う必要は通常ありません。
