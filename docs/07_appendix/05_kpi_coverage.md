...
本書で設計したデータモデルを使って、どのSaaS主要KPIが算出できるかを一覧で示します。算出できるものは計算式とソーステーブルを、算出できないものはその理由を記載します。
...
| **認識売上**（月次） | `SUM(scheduled_amount)` WHERE recognition_status IN ('recognized','scheduled') | `fact_revenue_schedule` |
| **繰延収益残高** | `SUM(deferred_balance)` | `fact_revenue_schedule` |
...
本書は「収益系システム」の設計書です。CAC・粗利率・Rule of 40 といった指標に必要なデータ（広告費・セールス人件費・COGS）は、HR系・マーケティング系・会計費用管理系のシステムから来ます。これらは収益系システムとは責務が異なる別ドメインであり、本書に費用系テーブルを追加することはスコープを超えます。

費用系の分析が必要な場合は、別途「費用ファクトテーブル」を設計し、DWH上で収益ファクトと結合する形で対応します。

| KPI | 算出に必要なデータ | 正本となるシステム |
|-----|----------------|----------------|
| **CAC**（顧客獲得コスト） | マーケティング費用 + セールス人件費（月次） | 広告プラットフォーム・HR系 |
| **Payback Period** | CAC + 粗利率 | 同上 |
| **LTV:CAC 比率** | CAC | 同上 |
| **粗利率 / Gross Margin** | COGS（ホスティング費・サポート人件費など） | ERP費用管理・HR系 |
| **Rule of 40** | 営業利益率 または EBITDA マージン | ERP費用管理 |
| **Magic Number** | 直近四半期のS&Mコスト + Net New ARR | 広告プラットフォーム・HR系 |

---

### プロダクト利用の指標——プロダクトアナリティクスのスコープ

DAU・ヘルススコアといった指標は、ログイン・操作・機能利用のイベントデータが必要です。これらはプロダクトアナリティクス（Mixpanel・Amplitude・Segment等）が担う領域であり、収益系システムとは設計の起点が異なります。収益系DWHにプロダクト利用データを取り込むことは技術的には可能ですが、それはプロダクトアナリティクスの設計書として別途扱うべきテーマです。

| KPI | 算出に必要なデータ | 正本となるシステム |
|-----|----------------|----------------|
| **DAU / MAU**（日次・月次アクティブユーザー） | ログイン・操作イベントログ | プロダクトアナリティクス |
| **機能別採用率**（Feature Adoption Rate） | 機能単位の利用イベントログ | プロダクトアナリティクス |
| **Time to Value** | オンボーディング完了・初回コアアクション到達イベント | プロダクトアナリティクス |
| **ヘルススコア** | 利用頻度・深度・サポートチケット数・NPS等の複合データ | CS系（Gainsight等）+ プロダクトアナリティクス |
| **トライアル→有料転換率** | トライアル開始イベント（`fact_mrr` ではMRR=0顧客の収録が任意のため） | プロダクトアナリティクス + Billing |

---

### 顧客フィードバックの指標——CSツールのスコープ

NPS・CSATはアンケート回答データを必要とし、Gainsight・Intercom・SurveyMonkey等のCSツールが正本を持ちます。収益系システムとは独立したデータソースです。

| KPI | 算出に必要なデータ | 正本となるシステム |
|-----|----------------|----------------|
| **NPS**（ネットプロモータースコア） | アンケート回答データ（0〜10点スコア） | CSツール（Gainsight等） |
| **CSAT**（顧客満足度スコア） | サポートチケット・アンケートデータ | CSツール・サポートツール |
...
```
算出できる（本書のモデルでカバー）
  ├── 収益    MRR / ARR / ARPU / ACV / MRRブリッジ / 認識売上 / 繰延収益残高
  ├── 解約    Revenue Churn / Logo Churn / GRR / NRR / コホート継続率
  ├── 成長    Quick Ratio / LTV（近似）
  └── 営業    Win Rate / 割引率 / DSO / 回収率

算出できない（スコープ外の理由別）
  ├── 費用系システムのデータが必要   CAC / Payback / LTV:CAC / 粗利率 / Rule of 40
  ├── プロダクトアナリティクスが必要  DAU/MAU / 機能採用率 / TTV / ヘルススコア / 転換率
  └── CSツールのデータが必要        NPS / CSAT
```

収益・解約・成長の主要指標は本書のモデルでカバーできます。コスト効率（CAC・Payback）とプロダクト利用（DAU・ヘルススコア）は、それぞれ費用系・プロダクト系の専門領域として別途設計が必要です。
